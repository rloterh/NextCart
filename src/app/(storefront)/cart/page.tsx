      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-3">
          ...
          <AnimatePresence mode="popLayout">
            {items.map((item) => {
              ...
              return (
                ...
                >
                  <div className="relative h-28 w-24 shrink-0 overflow-hidden bg-stone-100 dark:bg-stone-800">
                  ...
                  <div className="flex flex-1 flex-col justify-between">
                    <div className="flex justify-between">
                      <div>
                        ...
                        {item.variant ? <p className="text-xs text-stone-500">{item.variant.name}</p> : null}
                      </div>
                      <button type="button" onClick={() => removeItem(item.product.id, item.variant?.id)} className="p-1 text-stone-400 hover:text-stone-700">
                        <X className="h-4 w-4" />
                      </button>
                    </div>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center border border-stone-200 dark:border-stone-700">
                        <button type="button" onClick={() => updateQuantity(item.product.id, item.quantity - 1, item.variant?.id)} className="flex h-8 w-8 items-center justify-center text-stone-500">
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-24 border border-stone-200 bg-white p-6 dark:border-stone-800 dark:bg-stone-900">
            ...
            <Link href="/checkout">
              <Button className="mt-6 w-full" size="lg" rightIcon={<ArrowRight className="h-4 w-4" />}>
                Checkout
              </Button>
            </Link>
          </div>
