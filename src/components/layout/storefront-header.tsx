...
export function StorefrontHeader() {
  ...
  const compareCount = useDiscoveryStore((state) => state.compareProducts.length);
  const [mobileOpen, setMobileOpen] = useState(false);
  const [searchOpen, setSearchOpen] = useState(false);
  const [searchValue, setSearchValue] = useState("");

  const compareLabel = useMemo(() => (compareCount > 0 ? `Compare (${compareCount})` : "Compare"), [compareCount]);
    ...
    <>
      ...
      <header className="sticky top-0 z-50 border-b border-stone-200/60 bg-white/95 backdrop-blur-md dark:border-stone-800 dark:bg-stone-950/95">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
          <button onClick={() => setMobileOpen((current) => !current)} className="p-1 lg:hidden">
          ...
          <nav className="hidden items-center gap-8 lg:flex">
            {navLinks.map((link) => {
              const label = link.href === "/compare" ? compareLabel : link.label;
              const isActive = pathname === link.href || (link.href !== "/" && pathname.startsWith(link.href) && link.href !== "/shop?sort=newest");
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    isActive ? "text-stone-900 dark:text-white" : "text-stone-500 hover:text-stone-900 dark:text-stone-400 dark:hover:text-white"
                  )}
                >
                  {label}
                </Link>
        ...
        <div className="fixed inset-0 z-40 bg-white pt-28 dark:bg-stone-950 lg:hidden">
          <nav className="flex flex-col items-center gap-6 py-8">
            {navLinks.map((link) => (
              ...
              >
                {link.href === "/compare" ? compareLabel : link.label}
              </Link>
            ))}
          </nav>
        </div>
